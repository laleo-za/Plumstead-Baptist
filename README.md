...
- **About page**: Church story and beliefs; one main card with text and an image placeholder.
- **Contact page**: Phone, email, office hours, address, and map.
- **Sermons page**: List of recent series and placeholder cards for video/audio embeds.

Shared across the site:

- **Header**: Same blue bar with nav links (Home, Visit, About, Contact, Sermons) and logo on every page. The logo links to Home. The bar stays visible when you scroll and shrinks slightly.
...
| Path | Purpose |
|------|--------|
| **`app.py`** | Flask app: defines URLs (/, /visit, /about, /contact, /sermons) and fetches the latest YouTube video ID for the home page. Add or change routes here. |
...
| **`templates/sermons.html`** | Sermons page: list of series and embed placeholders. |
...
- **Visit, About, Contact, Sermons**: Edit the corresponding file in **`templates/`** (e.g. `visit.html`, `about.html`). Each template has a short comment at the top describing the page.
