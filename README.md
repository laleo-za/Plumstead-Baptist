...
| **`build_static.py`** | Builds the published site. Renders every template into a plain HTML file inside `docs/`, copies `static/` across, and writes a `robots.txt` plus a `sitemap.xml` so search engines can index the site. The first few lines of the file contain a `SITE_URL` constant (and a list of pages) that you may need to edit — see _Before going live_ below. Normally run automatically by the GitHub Action; you only need to run it by hand if you want to preview the exact static HTML or publish without waiting for the Action. |
...
| **`docs/`** | Auto-generated published site (output of `build_static.py`). **Do not edit by hand** — anything you change here will be overwritten the next time the build runs. This is the folder GitHub Pages serves. Includes a generated `robots.txt` (tells search engines what they may crawl) and `sitemap.xml` (lists the four public pages with a fresh "last modified" date on every build). |
| **`.github/workflows/rebuild.yml`** | GitHub Actions workflow that rebuilds `docs/` automatically — every 6 hours, whenever you push template/CSS changes to `main`, and on demand from the Actions tab. Edit this file if you ever want to change how often the site refreshes. |
...
The live site is hosted on **GitHub Pages**, which serves the contents of the `docs/` folder from the `main` branch. A GitHub Action (see `.github/workflows/rebuild.yml`) takes care of rebuilding `docs/` for you — you do **not** need to run `build_static.py` yourself.
...
3. **Commit and push** to GitHub:
   ...
   git push
   ```
4. **GitHub does the rest**: the workflow runs `build_static.py` on its servers, commits the regenerated `docs/` back to `main`, and GitHub Pages re-publishes the site within a minute or two.

### How the automatic rebuild works
...
- **Whenever you push** template, CSS, or other source changes to `main`. Push-triggered runs rebuild straight away rather than waiting for the next 6-hour tick.
- **On demand**, from the **Actions** tab on github.com: open _Rebuild and publish site_ and click _Run workflow_.

You can watch any run in progress on the **Actions** tab. If the YouTube feed is unreachable when the workflow runs, it will leave the previous (working) `docs/` in place and try again on the next scheduled run, so the live site never falls back to a placeholder.

### Heads-up about the bot’s commits

Because the workflow commits the rebuilt `docs/` back to the repository, your local copy of `main` will fall behind the remote whenever the bot runs. Get into the habit of running `git pull` before you start editing each time, otherwise your next `git push` will be rejected as "non-fast-forward".

### Manual rebuild (optional)

If you ever want to rebuild `docs/` yourself — for example to inspect the exact HTML that will go live — you can still do it locally:

```powershell
python build_static.py
```

This is also a perfectly valid way to publish: just `git add docs/` along with your other changes and push. The workflow will detect that `docs/` already matches the templates and skip its own commit.

If you want to preview the **exact** static files GitHub Pages will serve (rather than the Flask preview), serve the `docs/` folder with Python's built-in web server and open the address it prints:
...
The empty `.nojekyll` file inside `docs/` is created automatically by `build_static.py` and tells GitHub Pages to serve the folder as-is without running Jekyll on it.
...
- **Pull before you start editing**: The GitHub Action commits the rebuilt `docs/` back to the repo, so your local copy goes out of date as soon as the workflow has run. Run `git pull` before you start editing each session to avoid push conflicts.
